...
BACKGROUND_IMAGE_FILE = "menu_bg.jpeg"
...
# NOVO: Versão do Jogo
GAME_VERSION = "Game Version 1.0.0"
...
def draw_button(tela, fonte, rect, text, mouse_pos):
    ...
    cor_hover = (60, 60, 60)
    cor_fundo = cor_hover if rect.collidepoint(mouse_pos) else cor_padrao
...
def main_menu(tela, relogio, background_img, game_logo):
    ...
    #posição dos botões na gui
    btn_new_game_rect = pygame.Rect(TELA_LARGURA//5 - 250, altura_inicial, 500, 90)
    btn_load_game_rect = pygame.Rect(TELA_LARGURA//5 - 250, altura_inicial + 110, 500, 90)
    btn_exit_rect = pygame.Rect(TELA_LARGURA//5 - 250, altura_inicial + 220, 500, 90)
    ...
    while running and current_state == "MENU_PRINCIPAL":
        ...
        logo_rect.center = (TELA_LARGURA // 5, logo_y_center)
        ...
        # NOVO: Desenha a versão do jogo
        version_surf = fonte_versao.render(GAME_VERSION, True, COR_BRANCA)
        version_rect = version_surf.get_rect(center=(TELA_LARGURA // 2, TELA_ALTURA - 20))
...
def new_save_window(tela, relogio, background_img):
    ...
    INPUT_X = TELA_LARGURA // 5 - INPUT_W // 2
    TITLE_Y = 80
    INPUT_START_Y = 150
    INPUT_SPACING = 100
    ...
    btn_create_rect = pygame.Rect(TELA_LARGURA // 5 + 50, TELA_ALTURA - BTN_MARGIN_BOTTOM, 200, BTN_HEIGHT)
    btn_cancel_rect = pygame.Rect(TELA_LARGURA // 5 - 250, TELA_ALTURA - BTN_MARGIN_BOTTOM, 200, BTN_HEIGHT)
...
def main():
    global current_state, class_data, player_responses, background_img, game_logo, relogio, tela
    ...
    # -----------------------------------------------------
    # 🌟 CORREÇÃO CRÍTICA: LOOP PRINCIPAL DE JOGO FALTANDO 🌟
    # -----------------------------------------------------
    ...
    while True:
        if current_state == "MENU_PRINCIPAL":
            ...
            # Temporário: se entrar em GAMEPLAY, saia (ou coloque o loop real do jogo aqui)
            pygame.quit()
            sys.exit()
